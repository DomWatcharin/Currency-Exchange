...
def get_start_period():
    ...
    try:
        ...
        logging.info(f"Row count for bank_id 4: {row_count}")
        if row_count == 0:
...
def get_exchange_rates(api_key, date_str="06/01/2025", currency_type="2", language="th", timeout=10, max_retries=3):
    ...
    while retries < max_retries:
        try:
            ...
            end_time = time.time()
            logging.info(f"Time taken to fetch data for {date_str}: {end_time - start_time} seconds")
            response.raise_for_status()
            return response.json()
        except requests.exceptions.Timeout:
            logging.warning(f"Request timed out. Retrying... (attempt {retries + 1}/{max_retries})")
            retries += 1
            time.sleep(2)
        except requests.exceptions.RequestException as e:
            logging.error(f"Error during the request: {e}. Retrying... (attempt {retries + 1}/{max_retries})")
            retries += 1
            time.sleep(2)
    logging.error(f"Failed to retrieve exchange rate data after {max_retries} attempts.")
    send_notification(
        "BBL Exchange Rates Unavailable",
        f"Failed to fetch BBL exchange rates for {date_str} after {max_retries} attempts."
    )
    return None
